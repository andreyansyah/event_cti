...
!function($) {
	...
	var Iconpicker = function(element, options) {
			if ("undefined" == typeof $.fn.popover || "undefined" == typeof $.fn.popover.Constructor.VERSION)
	...
	Iconpicker.prototype.bindEvents = function() {
			...
			op.table.find(".btn-icon").off("click").on("click", function(e) {
					e.preventDefault(),
					el.select($(this).val()),
					op.inline === !1 ? el.$element.popover("3.x" === $.fn.bsVersion() ? "destroy" : "dispose") : op.table.find("i[class$='" + $(this).val() + "']").parent().addClass(op.selectedClass)
			}),
	...
	Iconpicker.prototype.removeAddClass = function(target, remove, add) {
			return this.options.table.find(target).removeClass(remove).addClass(add),
			add
	}
	,
	...
	Iconpicker.prototype.select = function(icon) {
			...
			-1 === op.selected && (op.selected = 0,
			icon = op.iconClassFix + op.icons[op.selected]),
			"" !== icon && op.selected >= 0 && (op.icon = icon,
			op.inline === !1 && (el.find("input").val(icon),
			el.find("i").attr("class", "").addClass(op.iconClass).addClass(icon)),
			icon === op.iconClassFix ? el.trigger({
					type: "change",
					icon: "empty"
			}) : (el.trigger({
					type: "change",
	...
	Iconpicker.prototype.switchPage = function(icon) {
			var op = this.options;
			if (op.selected = $.inArray(icon.replace(op.iconClassFix, ""), op.icons),
			op.selected >= 0) {
	...
	Iconpicker.prototype.updateArrows = function(page) {
			...
			1 === page ? op.table.find(".btn-previous").addClass("disabled") : op.table.find(".btn-previous").removeClass("disabled"),
			page === total_pages || 0 === total_pages ? op.table.find(".btn-next").addClass("disabled") : op.table.find(".btn-next").removeClass("disabled")
	}
	,
			...
			for (var i = 0; length > i; i++) {
					for (var tr = $("<tr></tr>"), j = 0; j < op.cols; j++) {
							...
							if (pos < op.icons.length) {
									...
									op.icon === v && btn.addClass(op.selectedClass).addClass("btn-icon-selected")
							}
	...
	Iconpicker.prototype.updateLabels = function(page) {
			...
			op.table.find(".icons-count").html(op.labelFooter.replace("{0}", total_icons ? offset + 1 : 0).replace("{1}", total_icons > total ? total : total_icons).replace("{2}", total_icons)),
			...
			if (op.header === !0) {
					for (var tr = $("<tr></tr>"), i = 0; i < op.cols; i++) {
							...
							if (0 === i || i === op.cols - 1) {
									...
									0 === tr.find(".page-count").length && (td.attr("colspan", op.cols - 2).append('<span class="page-count"></span>'),
									tr.append(td))
					}
	...
	Iconpicker.prototype.setIconset = function(value) {
			var op = this.options;
			$.isPlainObject(value) ? (Iconpicker.ICONSET._custom = $.extend(Iconpicker.ICONSET_EMPTY, value),
			op.iconset = "_custom") : Iconpicker.ICONSET.hasOwnProperty(value) ? op.iconset = value : op.iconset = Iconpicker.DEFAULTS.iconset,
	...
	$.fn.iconpicker = function(option, params) {
			return this.each(function() {
					...
					"string" == typeof option) {
							...
							"BUTTON" === $this.prop("tagName") ? ($this.empty().append("<i></i>").append('<input type="hidden" ' + name + "></input>").append('<span class="caret"></span>').addClass("iconpicker " + ("3.x" === $.fn.bsVersion() ? "" : "dropdown-toggle")),
